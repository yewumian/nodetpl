...
NodeTpl.prototype.express = {
  render: function(filepath, data, callback) {
    ...
    cache = that.cache[filepath];
    if (!cache) {
      require('fs').readFile(filepath, 'utf-8', function(err, content) {
        if (err) {
          callback(err);
        } else {
          that.cache[filepath] = content;
      ...
      content = nodetpl.render(cache, data, function(d) {
        callback(null, content);
      });
    }
  }
