...
/**
 * render a template file with data and callback
 * @method renderFile
 ...
 */
NodeTpl.prototype.renderFile = function(filepath, data, callback) {
  var that = this,
    cache = that._cache;
  if (cache[filepath]) {
    this._getJs(filepath, data, callback);
  } else {
    fs.readFile(filepath, 'utf-8', function(err, content) {
      if (err) {
        ...
        var context, result;
        content = content.replace(/(require\s*\(\s*['"])(.\/[^'"]+)/g, function(all, $1, $2) {
          return $1 + path.resolve($2);
        });
        cache[filepath] = {
          html: content
        };
        ...
        result = that._compile(filepath, {
          main: {
            html: that._html(content.trim(), 'main'),
          }
        });
        new vm.Script(result).runInContext(context);
        that._getJs(filepath, data, callback);
      }
    });
  }
};
...
NodeTpl.prototype.express = {
  render: function(filepath, data, callback) {
    ...
    nodetpl.renderFile(filepath, data, function(d) {
