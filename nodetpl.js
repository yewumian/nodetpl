...
(function(root, factory) {
  ...
  NodeTpl.prototype._html = function(content, tplname) {
    if (content) {
      ...
      var html = this._split(content, new RegExp('(' + openTag + '[\\s\\S]*?' + closeTag + ')'));
  ...
  NodeTpl.prototype._compile = function(cache) {
    ...
    for (var i in cache) {
      ...
      temp += '    var _ = \'\';\n';
      temp += '    var duid = that.duid();\n';
      temp += '    guid = guid || that.guid();\n';
      if (cache[i].css) {
      ...
      if (cache[i].js) {
        ...
        temp += 'var $TPLS = that.tpls;\n';
        temp += 'var $DATA = that.datas[duid];\n';
        temp += cache[i].js;
        temp += '  }';
        scripts.push(temp);
      }
  ...
  NodeTpl.prototype.render = function(html, data, callback) {
    ...
    new Function('iife', 'callback', result)(true, function(tpl) {
      callback && callback(tpl.render(data));
    });
    // jshint ignore: end
  };
